...
class Planet:
    def __init__(self, canvas, name, radius, color, speed, orbit_radius):
        ...
        self.planet_id = self.canvas.create_oval(0, 0, 0, 0, fill=self.color, outline='black')
...
class SolarSystemApp:
    def __init__(self, root):
        ...
        self.sun = self.canvas.create_oval(WIDTH / 2 - SUN_RADIUS, HEIGHT / 2 - SUN_RADIUS,
                                           WIDTH / 2 + SUN_RADIUS, HEIGHT / 2 + SUN_RADIUS, fill='yellow', outline='orange', width=3)

        self.planets = []
        self.speed_scales = []
        for name, color in PLANET_COLORS.items():
            speed = PLANET_SPEEDS[name]
            orbit_radius = (PLANET_RADIUS[name] + 45) * (list(PLANET_COLORS.keys()).index(name) + 1)
            planet = Planet(self.canvas, name, PLANET_RADIUS, color, speed, orbit_radius)
            self.planets.append(planet)
            scale = tk.Scale(self.canvas, from_=0.1, to=1.0, resolution=0.1, length=200, orient='horizontal', label=f'{planet.name} Speed', command=self._update_speeds)
            scale.set(speed)
            scale.bind("<ButtonRelease-1>", self._update_speeds)
            self.speed_scales.append(scale)
            self.canvas.create_window(10, 10 + (50 * (len(self.planets) - 1)), anchor='nw', window=scale)
